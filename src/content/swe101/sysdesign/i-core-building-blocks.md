...
- データ モデル、API コントラクト、障害モードについて話し合います。
...
- L4 (TCP): 高速で、コンテンツを認識しません。 AWS NLB。
...
NoSQL — 4 つの主要なファミリー:
- キーと値: Redis、DynamoDB — O(1) get/set;単純なクエリのみ。
...
PACELC 拡張機能: パーティションがなくても、待ち時間と一貫性が関係します。
トレードオフ — 低レイテンシには非同期レプリケーションが必要 → 一貫性が弱くなる。

## 8. 覚えてリハーサルする
- URL 短縮ツールを設計するための 5 ステップのアプローチを順を追って説明します。
- 1 億 DAU および 1 ユーザーあたり 1 日あたり 5 アクションのサービスの QPS を推定します。
