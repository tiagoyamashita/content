...
|注 |フォーカス |
|------|----------|
| [ボトルネックの特定](ii-identifying-bottlenecks.md) |リトルの法則、USL、USE、RED、リミッターの発見 |
| [CPU とメモリ](iii-cpu-and-memory.md) | CPU、GC、リーク、キャッシュ圧力 |
| [I/O & ストレージ](iv-io-and-storage.md) |ディスク、SSD、オブジェクトストレージ、WAL |
| [ネットワーク](v-network.md) |帯域幅、遅延、接続、メッシュ |
| [データベース](vi-database.md) |読み取り、書き込み、プール、インデックス |
| [アプリケーションレベル](vii-application-level.md) |雷鳴の群れ、ホットキー、サーキットブレーカー |
| [エリミネーションプレイブック](viii-elimination-playbook.md) |測定 → 分離 → 修正 → 検証 → 防止 |

## モグラたたきメンタルモデル

<figure class="notes-diagram"><svg xmlns="0 viewBox="0 0 440 100" role="img" aria-label="Bottleneck moves after each fix">
