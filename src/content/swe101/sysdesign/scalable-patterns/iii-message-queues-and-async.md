...
<figure class="notes-diagram"><svg xmlns="3 viewBox="0 0 460 140" role="img" aria-label="Sync call vs async queue decoupling">
...
<figure class="notes-diagram"><svg xmlns="4 viewBox="0 0 440 130" role="img" aria-label="Transactional outbox pattern">
...
| 2 |リレーは `outbox WHERE sent = false` を読み取ります |
| 3 |ブローカーに公開します。行を送信済みとしてマーク (または削除) |

## 5. 順序付けと分割

- **Kafka:** 順序保証 **パーティションごと** — 関連イベントのパーティション キー (例: `user_id`) を選択します。
- **グローバル順序:** 単一パーティション - スループットを制限します。
- **有害メッセージ:** 最大再試行後 → DLQ + アラート;キュー全体をブロックしないでください。
...
|多くの加入者へのファンアウト |低遅延 SLA を備えたシンプルな CRUD |

**関連:** [分散トランザクション](vii-distributed-transactions.md) (佐賀イベント)、[検索システム](v-search-systems.md) (CDC からインデックスへ)。
