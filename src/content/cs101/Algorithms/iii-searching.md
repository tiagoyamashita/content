...
## 2. 二分探索
**並べ替えられた**配列 (または `Comparator` で並べ替えられた順序) が必要です。
...
**一般的なバグ:** 一部の言語では `mid = (lo + hi) / 2` がオーバーフローする可能性があります。 **`lo + (hi - lo) / 2`** を使用してください。

## 3. 答えの二分探索 (パターン)
述語 `P(x)` が false から true (単調) に反転するような **最小 x** を求める問題の場合、配列インデックスではなく、範囲内の **x** に対して二分探索を行います。
...
|タスク | JDK |
|------|-----|
|ソートされた配列の検索 | `Arrays.binarySearch` (最初に並べ替え) |
|メンバーシップをリストする | `list.contains`、または多くのクエリの場合は `HashSet` |
