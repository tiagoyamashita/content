...
要素を**非減少**の順序(または**`Comparator`**)で配置します。 **比較ソート**では**compare**のみが使用され、特別なキー構造は使用されません。
...
- **Top-k / 部分順序:** ヒープまたは `PriorityQueue`、フル ソートではありません。
...
|必要 | API |
|------|-----|
|並べ替え `int[]` / `double[]` | `Arrays.sort` |
| `Object[]` または `List` で並べ替え | `Arrays.sort`、`list.sort(Comparator)`、`Collections.sort` |
