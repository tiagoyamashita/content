...
|-------|------|
| **主キー (PK)** |一意の行 ID (`id`) |
| **外部キー (FK)** |別のテーブルの PK を参照します (`orders.user_id → users.id`) |
...
**JOIN** はキーに基づいてテーブルを結合します。これが、エンティティが多対多に関連する場合に SQL を選択する主な理由です。
...
インデックスがない場合、**`WHERE email = ?`** はすべての行 (**O(n)**) をスキャンします。 `email` の **B ツリー** インデックスは、おおよそ **O(log n)** のポイント検索を行います。
...
| **2NF** |コンポジット PK に部分的な依存関係はありません。
| **3NF** |非キー列は別の非キー列に依存しません。

**非正規化** — 意図的にデータを重複させる — ストレージと更新の複雑さを引き換えに読み取り速度を実現します (分析では一般的ですが、必要性が測定されるまで OLTP ではまれです)。
...
- 1 つの巨大なテーブルでの **スケールアウト書き込み**は困難です (シャーディングは手動または NewSQL 経由で行われます)
