...
For system-design vocabulary (services, caches, queues), see [Core building blocks](../sysdesign/i-core-building-blocks.md). For Git workflow when diagrams live in repos, see [Git essentials](../git/essentials/i-overview.md). For UML-heavy diagrams with `!include` trees, see [PlantUML](../languages&frameworks/plantuml/i-overview.md).
