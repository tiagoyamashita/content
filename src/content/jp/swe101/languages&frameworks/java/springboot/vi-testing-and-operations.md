...
安全な **`/actuator/**`** 実際のデプロイメントでは Spring Security を使用 - 決して離れることはありません **`prometheus`** または **`env`** 公共のインターネット上で公開されます。 **セキュリティの基本とフィルター チェーン** [基本とフィルター チェーン](x-security-basics-and-filter-chain.md）。
