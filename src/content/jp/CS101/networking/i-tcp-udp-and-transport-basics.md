---
label: "I"
subtitle: "TCP、UDP、およびトランスポートの基本"
group: "ネットワーク"
order: 1
---
インターネット作業パート I: TCP、UDP、およびトランスポートの基本
...
- **小さいヘッダー、低遅延:** DNS クエリ、VoIP、ゲーム、時折の損失が許容されるメトリクスに適しています。

**UDPを選択する場合**

- **低レイテンシ**が必要で、損失を許容できるか、独自の信頼性を実装します (例: QUIC は UDP 上に構築されます)。
...
1. クライアントは **SYN** を送信します (シーケンス番号を同期します)。
2. サーバーは **SYN-ACK** と応答します。
3. クライアントは **ACK** を送信します — データ転送のために確立されたと見なされる接続 (詳細はスタックおよび TCP オプションによって異なります)。
...
- **HTTP/1.1 および HTTP/2** は、歴史的に TCP にありました。スタックがバイト ストリームの損失と順序付けを処理できるようにする必要があります。
...
**ソケット** は、プログラムが使用する API 境界です。サーバーではバインド/リッスン/受け入れ、クライアントでは接続/送信/受信を行います。カーネルは、ソケットを **プロトコル** (TCP または UDP)、**ローカル** および **リモート** IP/ポートのペア、およびバッファーに関連付けます。
...
|ネットワーク上のセキュリティ | TLS (多くの場合、TCP の上に) |
|ネーミング | DNS は名前 → アドレスをマップします。
...
次: **HTTP** (アプリケーション セマンティクス)、次に **TLS** (回線上の暗号化と ID)。
