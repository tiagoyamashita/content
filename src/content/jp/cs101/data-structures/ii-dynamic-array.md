...
Java では、固定サイズの配列を使用する場合、サイズは作成時に決定され、**変更できません**。  
- 最後の有効なインデックスを超える値を割り当てようとした場合 (例:`arr[4] = 42`長さ 4 の配列の場合、Java は ** をスローします`ArrayIndexOutOfBoundsException`** 実行時。
- **自動サイズ変更、コピー、拡大はありません**。配列のサイズは固定されており、それを「拡張」したい場合は、新しい（より大きな）配列を手動で作成し、要素を自分でコピーする必要があります。

#### 動的配列 (`ArrayList<E>`)

Java は、`ArrayList<E>`動的配列として動作するクラス:
- 電話をかけるとき`add(e)`そして、基礎となる配列が*いっぱい*ではありません(`size < capacity`)、Java は次のスロット (**O(1)** 時間) に要素を挿入します。
- 電話したら`add(e)`内部配列が*いっぱい*のとき(`size == capacity`)、Java は次のことを行います。
  1. **新しい、より大きな内部アレイを割り当てます** (デフォルトでは、Java 8+ では容量が最大 50% 増加します)。
  2. すべての既存の要素を新しい配列に **コピー**します。
  3. 古い配列を **解放**します (ガベージ コレクションのため)。
  4. 新しい要素を **追加**します。
- サイズ変更とコピーの手順は *高価* (`O(n)`n 要素の場合）、まれに発生するため、1 回あたりの平均時間は`add`**償却された O(1)** のままです。

#### エッジケース (Java)

- **初期容量ゼロ:**`ArrayList`初期容量を持たずにすぐに追加する場合、リストは最初の追加時にストレージを割り当てる必要があります。
- **繰り返し追加:** 短時間に多くの項目をプッシュすると、Java は、成長ポリシーに応じて、再割り当てとコピーを連続して数回行うことがあります。
- **最大配列サイズ:** Java 配列には最大サイズがあります (`Integer.MAX_VALUE`）。これを乗り越えて成長しようとすると、`OutOfMemoryError`。
- **一括追加 (`addAll`):** 多くの要素を一度に追加すると、すべての新しいアイテムに合わせてすぐにサイズ変更が行われる可能性があります。

#### *決めない*のか (Java)

- **固定サイズの配列**の場合、プッシュ (「過去の容量の追加」) によってサイズが変更されることはなく、例外がスローされます。
- のために`ArrayList`、Java はサイズ変更を自動的に処理しますが、時間がかかり、大量のメモリ割り当てが原因でガベージ コレクションの一時停止が発生する場合があります。
- カスタム ポリシー (追加するたびに常に +1 ずつサイズ変更するなど) の場合、パフォーマンスが 2 次まで低下する可能性があります。Java のデフォルトの方が効率的です。

---

- **典型的な操作:**`get`/`set`インデックス (**O(1)**);`add`**O(1)** の償却を終了します。シフトのため、中央 **O(n)** で挿入/削除します。
- **スペース:** Θ(n)`n`要素。実際の内部容量はさらに大きくなる可能性があります (サイズ変更のため、通常は n から約 1.5n の間です)。
