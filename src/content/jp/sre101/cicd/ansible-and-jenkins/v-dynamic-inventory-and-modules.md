...
静的 INI ファイルは固定フリートで機能します。 **動的インベントリ**は、**AWS、Azure、GCP**、またはカスタム スクリプトからライブ ホスト リストを取得します。
...
|メリット |詳細 |
|----------|----------|
|自動検出 | `Role=web` タグが付いた新しい EC2 インスタンスがグループに参加 |
...
健全性が決して合格しない場合、プレイブックは失敗します。Jenkins ステージは失敗し、サイレント不良デプロイは行われません。
