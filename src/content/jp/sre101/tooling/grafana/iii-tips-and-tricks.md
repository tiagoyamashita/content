...
order: 3
---
SRE ツール — Grafana: ヒントとテクニック

ダッシュボードとアラートを使いやすく保つ、オペレーターを中心とした習慣。

## 1. ゴールデンシグナル

ユーザー向けの重要なサービスごとに、**遅延**、**トラフィック**、**エラー**、**飽和度** (または合意された RED/USE スライス) をカバーするパネルを目指します。サービス全体で同じレイアウトを使用することで、オンコールの筋肉の記憶が高速化されます。
...
- 意図的に階層化しない限り、**Prometheus** および **Grafana** の統合アラートで同じ条件が重複することを避けます。
...
- エクスポートされた JSON を Git と定期的に比較して、サイレントな UI 編集をキャッチします。
