...
|従来のプログラミング |機械学習 |
|-----------------------|------|
|エンジニアは `if` / `else` ロジックを作成します |モデルはデータからパターンを学習します |
...
ML は魔法ではありません。モデルが一般化しているのか、単に記憶しているのかを知るには、**代表的なデータ**、**明確な目的**、**評価**が必要です。
...
現在の本番環境の ML のほとんどは、**教師あり** または **自己教師あり** (データ自体から派生したラベル、言語と視覚の事前トレーニングで一般的) です。
...
語彙とワークフローについてはここから始めてください。プロジェクトを構築するときに、特定のアルゴリズム、機能エンジニアリング、およびフレームワークに関するフォローアップ メモを追加します。常に理論を **データセット** および **メトリック**に結び付けてください。ML は読むだけではなく、実際に行うことで学習されます。

**関連:** **AI → ML Foundations** では、教師あり/教師なし手法と評価指標について詳しく説明しています。
